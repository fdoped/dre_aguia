...
[data-testid="stSidebar"] * { color: #8FAEC8 !important; font-family: 'Inter', sans-serif !important; }
[data-testid="stSidebar"] h1 { 
    color: #E2E8F0 !important; font-family:system-ui, -apple-system, \"Segoe UI\", Roboto, Helvetica, Arial, sans-serif !important;
    font-size: 1.1rem !important; font-weight: 700 !important; letter-spacing: 0.05em !important;
}
[data-testid="stSidebar"] h2,
[data-testid="stSidebar"] h3 { color: #C8D9E9 !important; font-family:system-ui, -apple-system, \"Segoe UI\", Roboto, Helvetica, Arial, sans-serif !important; }
[data-testid="stSidebar"] label,
[data-testid="stSidebar"] p { color: #8FAEC8 !important; font-size: 0.8rem !important; font-weight: 500 !important; }
[data-testid="stSidebar"] [data-testid="stMarkdownContainer"] p { color: #A0B8CC !important; font-weight: 600 !important; font-size: 0.78rem !important; }
...
[data-testid="stSidebar"] [data-baseweb="input"] input { color: #CBD5E1 !important; }
[data-testid="stSidebar"] [data-baseweb="tag"] {
    background: #1A3A5C !important;
    border-radius: 5px !important;
}
[data-testid="stSidebar"] [data-baseweb="tag"] span { color: #7DD3FC !important; font-size: 0.72rem !important; }

/* Toggle */
[data-testid="stToggle"] label { color: #8FAEC8 !important; }
[data-testid="stToggle"] [data-checked="true"] { background: #2563EB !important; }
...
/* Divider */
[data-testid="stSidebar"] hr { border-color: #1A2E4A !important; margin: 10px 0 !important; }
...
[data-testid="stSidebar"] [data-testid="metric-container"] [data-testid="stMetricLabel"] p { color: #7B9BBB !important; font-size: 0.72rem !important; }
[data-testid="stSidebar"] [data-testid="metric-container"] [data-testid="stMetricValue"] { color: #E2E8F0 !important; font-size: 1rem !important; font-family: 'Inter', sans-serif !important; }

/* Main content text */
.stMarkdown p, .stText, h1, h2, h3 { color: #E2E8F0 !important; }
...
def build_dre_html(df_f: pd.DataFrame, df_orc: pd.DataFrame, df_orc_ano: pd.DataFrame,
    ...
    def _alert_badge(valor: float, orc: float) -> tuple:
        ...
        if var_pct >= var_threshold:
            ...
            badge = (
                ...
                f"style='display:inline-block;margin-left:6px;color:#22C55E;"
        ...
        if var_pct <= -var_threshold:
            ...
            badge = (
                ...
                f"onclick='event.stopPropagation();' "
                f"style='display:inline-block;margin-left:6px;color:#EF4444;"
                f"font-size:11px;cursor:help;vertical-align:middle;'>&#9888;</span>"
            )
        ...
        badge = (
            ...
            f"onclick='event.stopPropagation();' "
            f"style='display:inline-block;margin-left:6px;color:#F59E0B;"
    ...
    def _td_orc(label: str, valor: float, orc: float, orc_ano: float = 0.0,
        ...
        if prov is not None and not df_prov.empty:
            prov_cor = "#94A3B8" if prov == 0 else ("#22C55E" if prov > 0 else "#F87171")
            td_prov = f"<td class='td-prov' style='color:{prov_cor};'>{_fmt(prov) if prov != 0 else '—'}</td>"
        else:
            td_prov = "<td class='td-prov' style='color:#334155;'>—</td>"
        ...
        if orc_ano != 0:
            ...
            exe_cor = "#22C55E" if exe_pct >= 100 else ("#F59E0B" if exe_pct >= 75 else "#EF4444")
            td_exe = f"<td class='td-exe' style='color:{exe_cor};'>{exe_pct:.1f}%</td>"
        else:
            td_exe = "<td class='td-exe' style='color:#475569;'>—</td>"
        ...
        var_pct = (valor - orc) / abs(orc) * 100
        ruim = valor < orc
        cor_var = "#EF4444" if ruim else "#22C55E"
    ...
    def _pct_html(valor: float, mc: float, cls_extra: str = "") -> str:
        ...
        pct = (valor / mc) * 100
        cor = "#EF4444" if pct < 0 else "#22C55E"
        s = f"{abs(pct):.1f}%"
    ...
    def _chart_icon(chart_key: str) -> str:
        ...
        return (
            ...
            f"color:#3B82F6;font-size:11px;vertical-align:middle;opacity:.55;"
    ...
    def tr_subtotal(label: str, valor: float, is_final: bool = False, mc: float = None,
        ...
        if is_final:
            ...
            return (
                ...
                f"<td class='td-label td-total-label' style='color:#E2E8F0;'>(=) {label}{chart_html}{alert_html}</td>"
                f"{pct_final}"
                f"{td_meses}"
                f"<td class='td-val td-total-val' style='color:{cor_v};'>{_fmt(valor)}</td>"
                f"{td_orc_html}</tr>"
                f"<tr><td colspan='{total_cols}' style='height:4px;background:#060D1A;'></td></tr>"
            )
        cor_v = _cor(valor)
        return (
            ...
            f"{td_orc_html}</tr>"
            f"<tr><td colspan='{total_cols}' style='height:2px;background:#060D1A;'></td></tr>"
        )
...
html, body { width: 100%; background: #060D1A; font-family: 'Inter', sans-serif; font-size: 13px; color: #CBD5E1; }
...
/* ── COLUNA ESQUERDA — DRE ───────────────────────────── */
.cc-dre { overflow: hidden; border-right: 1px solid #1A2E4A; border-radius: 10px 0 0 10px; background: #080F1C; }
...
.td-pct   { width: 12%; padding: 0 8px; text-align: right; border: none;
            white-space: nowrap; font-variant-numeric: tabular-nums; font-size: 11px; font-style: italic; color: #64748B; }
...
.td-orc   { width: 18%; padding: 0 10px; text-align: right; border: none;
            white-space: nowrap; font-variant-numeric: tabular-nums;
            font-feature-settings: "tnum" 1; color: #3D5A78; font-size: 14px;
...
.td-prov  { width: 10%; padding: 0 10px; text-align: right; border: none;
            ...
            color: #64748B; letter-spacing: 0.01em; border-left: 1px solid #1E293B; }
.td-label { width: 40%; }
.td-pct   { width: 10%; }

/* cabeçalho de colunas */
thead tr { background: #060D1A; border-bottom: 1px solid #1A2E4A; }
thead th { padding: 8px 14px; font-size: 9.5px; font-weight: 700; text-transform: uppercase;
           letter-spacing: .12em; color: #3D5A78; text-align: right; white-space: nowrap; border: none; }
thead th:first-child { text-align: left; color: #4A6680; }
thead th.td-mes { color: #2A4560; font-size: 9px; }

/* seções */
.tr-sec { background: #06101E; }
.td-sec { padding: 6px 14px; color: #2A5080; font-size: 9px; font-weight: 700;
          letter-spacing: 0.18em; text-transform: uppercase; border: none; border-top: 1px solid #0F1E30; }

/* categoria nível 1 */
.tr-cat { background: #0B1627; border-top: 1px solid #111E30; cursor: pointer; transition: background .12s; }
.tr-cat:hover { background: #0E1C33; }
.td-cat-label { padding-top: 9px; padding-bottom: 9px; font-weight: 600; color: #C8D9EC; font-size: 13px; }

/* grupo nível 2 */
.tr-grp  { display: none; background: #09131F; border-top: 1px solid #0F1A28; cursor: pointer; transition: background .12s; }
.tr-grp:hover { background: #0C1828; }
.td-grp-label { padding: 7px 14px 7px 32px; color: #8FAEC8; font-weight: 600; font-size: 12.5px; }

/* subgrupo nível 3 */
.tr-subg { display: none; background: #07101C; border-top: 1px solid #0D1825; cursor: pointer; transition: background .12s; }
.tr-subg:hover { background: #0A1526; }
.td-subg-label { padding: 6px 14px 6px 50px; color: #607A90; font-size: 12px; }

/* folha nível 4 */
.tr-leaf { display: none; background: #060E1A; border-top: 1px solid #0C1824; cursor: pointer; transition: background .12s; }
.tr-leaf:hover { background: #091422; }
.td-leaf-label { padding: 5px 14px 5px 68px; color: #3D6080; font-size: 11.5px; }
...
.pos-val { color: #22C55E !important; }
.neg-val { color: #EF4444 !important; }

/* subtotais */
.tr-subtotal { background: #0D1E32; border-top: 1px solid #1A3050; border-bottom: 1px solid #1A3050; }
.td-subtotal-label { padding: 8px 14px; color: #A8C0D8; font-weight: 700; font-size: 12.5px; }
.td-subtotal-val   { padding: 8px 14px; font-weight: 700; font-size: 12.5px; }
.tr-total-final { background: #0A1E3A; border-top: 2px solid #1A3A60; }
.td-total-label { padding: 12px 14px; color: #E2E8F0; font-weight: 700; font-size: 13px; }
...
.td-mes {
  ...
  font-variant-numeric: tabular-nums; font-feature-settings: "tnum" 1;
  color: #3D5A78; font-size: 14px; font-weight: 400; background: rgba(6,13,26,.3);
...
/* seta */
.arrow    { display: inline-block; width: 14px; color: #2A4560; font-size: 10px;
...
.btn-meses {
  ...
  background: #0C1A2C; border: 1px solid #1A3050; border-radius: 5px;
  color: #4A7FA8; font-size: 10px; font-weight: 700; letter-spacing: .08em;
  padding: 5px 10px; cursor: pointer; margin: 6px 14px; text-transform: uppercase;
  transition: all .15s;
}
.btn-meses:hover { background: #0F2040; border-color: #2A5080; color: #7DD3FC; }
...
.cc-panel-top-title {
  ...
  color: #2A5080; margin-bottom: 1px;
}
.cc-panel-top-sub {
  font-size: 9.5px; color: #3D5A78; font-weight: 500;
}

/* KPI cards */
.cc-kpi-list { padding: 10px 10px 6px; display: flex; flex-direction: column; gap: 6px; }
.cc-kpi {
  background: #0C1825; border-radius: 7px; padding: 9px 11px;
  border: 1px solid #152030;
  border-left: 3px solid #1A3A60;
}
.cc-kpi-label { font-size: 9.5px; color: #3D5A78; font-weight: 700; text-transform: uppercase;
...
.cc-kpi-val.pos { color: #22C55E; }
.cc-kpi-val.neg { color: #EF4444; }
.cc-kpi-sub { font-size: 9.5px; color: #2A4560; margin-top: 3px; }
.cc-kpi-sub b { color: #3D6080; }

/* Divisor de painel */
.cc-panel-title {
  font-size: 8.5px; font-weight: 700; text-transform: uppercase; letter-spacing: .14em;
  color: #2A4560; padding: 10px 12px 5px; border-top: 1px solid #0F1E30; margin-top: 2px;
  background: #060D1A;
}
...
.cc-bar-header { display: flex; justify-content: space-between; align-items: baseline;
                 font-size: 11px; color: #607A90; margin-bottom: 4px; font-weight: 500; }
.cc-bar-header span:last-child { font-size: 11px; font-weight: 700; font-variant-numeric: tabular-nums; color: #8FAEC8; }
.cc-bar-track { height: 6px; background: #0F1E30; border-radius: 4px; overflow: hidden; }
...
.cc-wf-lbl { font-size: 8.5px; color: #3D5A78; text-align: center; white-space: nowrap;
             overflow: hidden; text-overflow: ellipsis; width: 100%; font-weight: 700; }
.cc-wf-val { font-size: 7.5px; color: #2A4060; text-align: center; white-space: nowrap; font-weight: 500;
...
#modal-header { background: #060D1A; border-bottom: 1px solid #1A3050; padding: 14px 20px; display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; }
#modal-title  { color: #CBD5E1; font-size: 14px; font-weight: 700; }
#modal-total  { color: #3D6080; font-size: 11.5px; margin-top: 2px; }
#modal-close  { background: none; border: none; color: #2A5080; font-size: 22px; cursor: pointer; line-height: 1; padding: 0 4px; }
#modal-close:hover { color: #7DD3FC; }
#modal-body { overflow-y: auto; flex: 1; background: #080F1C; }
#modal-table { width: 100%; border-collapse: collapse; font-size: 12px; }
#modal-table thead th { position: sticky; top: 0; background: #060D1A; padding: 8px 12px;
  text-align: left; font-weight: 700; color: #2A5080; font-size: 9.5px;
  text-transform: uppercase; letter-spacing: .08em; border-bottom: 1px solid #1A2E4A; white-space: nowrap; }
#modal-table thead th.num { text-align: right; }
#modal-table tbody tr { border-bottom: 1px solid #0C1828; }
#modal-table tbody tr:hover { background: #0A1525; }
#modal-table tbody td { padding: 7px 12px; color: #607A90; }
...
#modal-table tbody td.pos { color: #22C55E; }
#modal-table tbody td.neg { color: #EF4444; }
#modal-footer { background: #060D1A; border-top: 1px solid #1A2E4A; padding: 9px 20px;
  display: flex; justify-content: space-between; align-items: center; font-size: 11.5px; color: #2A4560; }
...
.alert-badge.alert-vermelho:hover { color: #F87171 !important; }
.alert-badge.alert-verde:hover    { color: #4ADE80 !important; }
.alert-badge.alert-amarelo:hover  { color: #FBBF24 !important; opacity: 1 !important; }

/* ── ÍCONE DE GRÁFICO INLINE ── */
.chart-icon { user-select: none; }
.chart-icon:hover { color: #60A5FA !important; opacity: 1 !important; }
...
#chart-modal-header {
  background: #060D1A; border-bottom: 1px solid #1A3050;
  padding: 14px 20px; display: flex; align-items: center;
  justify-content: space-between; flex-shrink: 0;
}
#chart-modal-title { color: #CBD5E1; font-size: 14px; font-weight: 700; }
#chart-modal-sub   { color: #3D6080; font-size: 11.5px; margin-top: 2px; }
#chart-modal-close { background: none; border: none; color: #2A5080; font-size: 22px; cursor: pointer; line-height: 1; padding: 0 4px; }
#chart-modal-close:hover { color: #7DD3FC; }
#chart-modal-body { padding: 18px 20px 14px; background: #080F1C; }
#chart-modal-legend {
  display: flex; gap: 18px; padding: 0 20px 10px;
  font-size: 11px; color: #607A90; background: #080F1C;
}
...
#chart-modal-footer {
  background: #060D1A; border-top: 1px solid #1A2E4A;
  padding: 8px 20px; font-size: 11px; color: #2A4560;
}
.chart-svg { width: 100%; height: 280px; display: block; }
.chart-svg .bar-real { fill: #3B82F6; }
.chart-svg .bar-orc  { fill: #475569; }
.chart-svg .grid-line { stroke: #1A2E4A; stroke-width: 1; stroke-dasharray: 2,3; }
.chart-svg .axis-text { fill: #4A6680; font-family: 'Inter', sans-serif; font-size: 10px; }
.chart-svg .bar-label { fill: #607A90; font-family: 'Inter', sans-serif; font-size: 9px; }
</style>
    ...
    try {
      ...
      window.parent.document.querySelectorAll('iframe').forEach(function(f) {
        if (f.contentWindow === window) {
          f.style.background = '#0B1120';
          f.style.colorScheme = 'dark';
          if (f.parentElement) f.parentElement.style.background = '#0B1120';
        }
      });
    } catch(e) {}
  }
...
function showChart(key){
  ...
  svg += '<line x1="' + padL + '" y1="' + y0 + '" x2="' + (W-padR) + '" y2="' + y0 + '" stroke="#2A4560" stroke-width="1"/>';
...
function toggleMeses(){
  ...
  btn.style.background = opening ? '#0A2040' : '#0C1A2C';
  btn.style.borderColor = opening ? '#2A5080' : '#1A3050';
}
...
<div id="modal-overlay" onclick="if(event.target===this)closeModal()">
  <div id="modal-box">
    ...
    <div id="modal-footer">
      ...
      <span>Pressione <kbd style="background:#0C1828;border:1px solid #1A3050;border-radius:3px;padding:1px 6px;font-size:10px;color:#4A7090;">Esc</kbd> para fechar</span>
    </div>
  </div>
</div>
...
<div id="chart-modal-overlay" onclick="if(event.target===this)closeChartModal()">
  <div id="chart-modal-box">
    ...
    <div id="chart-modal-legend">
      <span class="lg-it"><span class="lg-sw" style="background:#3B82F6;"></span> Realizado</span>
      <span class="lg-it"><span class="lg-sw" style="background:#475569;"></span> Orçado</span>
    </div>
...
<div class="cc-shell">
  ...
  <div class="cc-dre">
    <table id="dre-table" class="meses-off">
      <thead>
        <tr style="background:#060D1A;border-bottom:1px solid #1A2E4A;">
          <th class="td-label" style="padding:10px 14px;text-align:left;font-size:10px;font-weight:700;color:#2A5080;letter-spacing:.12em;text-transform:uppercase;border:none;white-space:nowrap;"><button id="btn-meses" onclick="toggleMeses()" style="background:#0C1A2C;border:1px solid #1A3050;border-radius:5px;color:#3D6080;font-size:9px;font-weight:700;letter-spacing:.05em;cursor:pointer;padding:3px 7px;margin-right:8px;vertical-align:middle;transition:all .15s;">▶ MESES</button>Descrição</th>
          <th class="td-pct"  style="padding:10px 8px;text-align:right;font-size:9.5px;font-weight:700;color:#2A5080;letter-spacing:.12em;text-transform:uppercase;border:none;white-space:nowrap;">% MC</th>
""" + 
    "".join(
        f'          <th class="td-mes" data-mes="{m}" style="padding:10px 8px;text-align:right;font-size:9px;font-weight:700;'
        f'color:#1E3A58;letter-spacing:.08em;text-transform:uppercase;border:none;white-space:nowrap;">{MESES_CURTO[m]}</th>'
        for m in meses_ativos
    ) + """
          <th class="td-val"  style="padding:10px 14px;text-align:right;font-size:9.5px;font-weight:700;color:#2A5080;letter-spacing:.12em;text-transform:uppercase;border:none;white-space:nowrap;">Total (R$)</th>
          <th class="td-orc"  style="padding:10px 10px;text-align:right;font-size:9.5px;font-weight:700;color:#2A5080;letter-spacing:.12em;text-transform:uppercase;border:none;white-space:nowrap;">Orçado (R$)</th>
          <th class="td-var"  style="padding:10px 10px;text-align:right;font-size:9.5px;font-weight:700;color:#2A5080;letter-spacing:.12em;text-transform:uppercase;border:none;white-space:nowrap;">Var%</th>
          <th class="td-exe"  style="padding:10px 8px;text-align:right;font-size:9.5px;font-weight:700;color:#2A5080;letter-spacing:.12em;text-transform:uppercase;border:none;white-space:nowrap;">Real/Orc Ano</th>
          <th class="td-prov" style="padding:10px 10px;text-align:right;font-size:9.5px;font-weight:700;color:#2A5080;letter-spacing:.12em;text-transform:uppercase;border:none;white-space:nowrap;border-left:1px solid #1E293B;">Prov.</th>
        </tr>
      </thead>
      <tbody>
""" + tbody + """
  ...
  <div class="cc-panel" id="side-panel">
    ...
    <div style="background:#060D1A;padding:10px 12px;border-bottom:1px solid #1A2E4A;border-radius:0 10px 0 0;">
      <div style="font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:.18em;color:#2A5080;">Command Center</div>
      <div style="font-size:10px;color:#3D5A78;margin-top:2px;font-weight:500;">Resumo Executivo</div>
    </div>
...
var parcelas = [
  { label:'Impostos',      val: Math.abs(MC - rl),   cor:'#D84B20' },
  { label:'Desp. Operac.', val: Math.abs(rl - ebit), cor:'#1E5FA8' },
  { label:'Financeiro',    val: Math.abs(ebit - liq), cor:'#0E6E56' },
];
var barHTML='';
...
<!-- ── LEGENDA DE DESVIOS ── -->
<div style="display:flex;align-items:center;gap:16px;padding:6px 4px 10px;font-family:'Inter',sans-serif;font-size:10px;color:#2A4060;">
  <span style="font-weight:700;color:#2A5080;text-transform:uppercase;letter-spacing:.08em;font-size:9px;">Desvios no mês atual:</span>
  ...
  <span style="color:#1A3050;font-size:9.5px;">(passe o mouse sobre o ícone para detalhes)</span>
</div>

<!-- ── LEGENDA DE ALERTAS VS ORÇADO + AÇÕES POR LINHA ── -->
<div style="display:flex;align-items:center;gap:16px;padding:0 4px 12px;font-family:'Inter',sans-serif;font-size:10px;color:#2A4060;flex-wrap:wrap;">
  <span style="font-weight:700;color:#2A5080;text-transform:uppercase;letter-spacing:.08em;font-size:9px;">Desempenho Real vs Orçado (banda ±""" + f"{int(var_threshold_safe)}" + """%):</span>
  <span style="display:inline-flex;align-items:center;gap:5px;">
    <span style="display:inline-block;width:3px;height:11px;background:#22C55E;"></span>
    <span style="color:#22C55E;font-size:11px;">&#9650;</span>
    Favorável: Real superou orçado em &ge; """ + f"{int(var_threshold_safe)}" + """%
  </span>
  <span style="display:inline-flex;align-items:center;gap:5px;">
    <span style="color:#F59E0B;font-size:11px;opacity:.7;">&#9679;</span>
    Dentro da tolerância: desvio &lt; """ + f"{int(var_threshold_safe)}" + """%
  </span>
  <span style="display:inline-flex;align-items:center;gap:5px;">
    <span style="display:inline-block;width:3px;height:11px;background:#EF4444;"></span>
  ...
  <span style="display:inline-flex;align-items:center;gap:5px;color:#3B82F6;">
    ...
    <span style="color:#2A4060;">Clique no ícone azul para evolução mensal vs orçado</span>
  </span>
</div>
...
.kpi-wrap-inner {
  flex: 1; background: #080F1C; border: 1px solid #111E30;
  border-radius: 9px; overflow: hidden; min-width: 0;
}
.kpi-head-inner {
  display: flex; align-items: center; gap: 8px;
  padding: 9px 12px 8px; background: #060D1A; border-bottom: 2px solid;
}
.kpi-head-dot-inner { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.kpi-head-title-inner {
  font-size: 10.5px; font-weight: 700; color: #A0B8CC;
...
.kpi-c-lbl-inner {
  font-size: 9px; font-weight: 700; color: #2A4560;
...
(function(){
  ...
  var grupos = [
    ...
    { titulo:'&#128184; Estrutura de Custos', cor:'#A0440A', kpis:[
    ...
    { titulo:'&#128200; Convers\u00e3o de Resultado', cor:'#0E6040', kpis:[
    ...
    { titulo:'&#127974; Valores do Per\u00edodo', cor:'#1E2E7C', kpis:[
...
<style>
.analise-wrap {
  margin-top: 20px; border: 1px solid #1A2E4A;
  border-radius: 10px; overflow: hidden; background: #080F1C;
}
.analise-header {
  display: flex; align-items: center; gap: 10px;
  padding: 11px 16px; background: #060D1A;
  border-bottom: 1px solid #1A2E4A;
  cursor: pointer; user-select: none;
}
.analise-header-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.analise-header-title {
  font-size: 11px; font-weight: 700; color: #C8D9EC;
  text-transform: uppercase; letter-spacing: .14em;
}
.analise-header-sub { font-size: 10px; color: #3D5A78; margin-left: auto; font-weight: 500; }
.analise-toggle { color: #2A5080; font-size: 11px; transition: transform .2s; margin-left: 10px; }
...
.at thead th {
  ...
  background: #06111E; border-bottom: 1px solid #1A2E4A;
  font-size: 9px; font-weight: 700; color: #2A5080;
  text-transform: uppercase; letter-spacing: .12em;
  white-space: nowrap; text-align: right;
}
.at thead th:first-child { text-align: left; min-width: 200px; }
.at tbody tr { border-bottom: 1px solid #0C1828; transition: background .1s; }
.at tbody tr:hover { background: #0A1525; }
.at tbody tr.at-ref { background: #07101C; }
.at tbody tr.at-sub { background: #0D1E32; border-top: 1px solid #1A3050; }
.at tbody tr.at-sub td { font-weight: 700; }
.at tbody td {
  padding: 8px 14px; color: #607A90; text-align: right;
  white-space: nowrap; font-variant-numeric: tabular-nums;
  font-feature-settings: "tnum" 1;
}
.at tbody td:first-child { text-align: left; color: #8FAEC8; font-weight: 500; }
.at tbody tr.at-ref td:first-child { color: #E2E8F0; font-weight: 700; }
.at tbody tr.at-sub td:first-child { color: #C8D9EC; }
.at-pos { color: #22C55E !important; }
.at-neg { color: #EF4444 !important; }
.at-neu { color: #2A4560 !important; }
.at-bar { display:inline-block;vertical-align:middle;width:40px;height:5px;
          background:#0F1E30;border-radius:3px;margin-left:6px;overflow:hidden; }
.at-bar span { display:block;height:100%;border-radius:3px; }
.ah-up   { color: #22C55E !important; font-weight: 600; }
.ah-dn   { color: #EF4444 !important; font-weight: 600; }
.ah-zero { color: #2A4560 !important; }
</style>
...
(function(){
  ...
  function miniBar(v){
    var p=Math.min(100,Math.abs(v)); var c=v>=0?'#22C55E':'#EF4444';
  ...
  avH+='<th style="border-left:1px solid #1A2E4A;color:#4A6680;">Total</th></tr></thead><tbody>';

  /* VB = base 100% */
  avH+='<tr class="at-ref"><td>Venda Bruta <span style="color:#2A4560;font-size:9px;">(base = 100%)</span></td>';
  MESES.forEach(function(){ avH+='<td class="at-neu">100.0%'+miniBar(100)+'</td>'; });
  avH+='<td style="border-left:1px solid #1A2E4A;color:#2A4560;">100.0%</td></tr>';
  ...
  SEQ.forEach(function(s){
    ...
    avH+='<td class="'+corCls(pTot)+'" style="border-left:1px solid #1A2E4A;">'+fmtP(pTot)+miniBar(pTot)+'</td>';
    avH+='</tr>';
  });
  avH+='</tbody>';
  ...
  if(MESES.length>=2){
    ahH+='<th style="border-left:1px solid #1A2E4A;">'+mnm(MESES[0])+' → '+mnm(MESES[MESES.length-1])
        +'<br><span style="color:#2A4560;font-size:8px;font-weight:400;">Acumulado</span></th>';
  }
  ahH+='</tr></thead><tbody>';
  ...
  if(MESES.length>=2){
    ...
    ahH+='<td style="border-left:1px solid #1A2E4A;">'+fmtMoM(v0?(vn-v0)/Math.abs(v0)*100:0)+'</td>';
  }
  ahH+='</tr>';
...
st.markdown(
    ...
    f"font-weight:700; color:#E2E8F0; display:flex; align-items:center; letter-spacing:-0.01em;'>"
    f"DRE Gerencial Consolidado "
    f"<span style='font-size: 1.3rem; color: #4A7FA8; font-weight: 500;'>{empresa_html_display}</span></h2>"
    f"<p style='margin:0; font-size:0.82rem; color:#3D5A78; letter-spacing:0.01em; font-family:system-ui, -apple-system, sans-serif;'>"
    f"📅 <b style='color:#4A7090'>{periodo_str}</b> &nbsp;|&nbsp; "
    f"📋 <b style='color:#4A7090'>{', '.join(cenarios_sel)}</b> &nbsp;|&nbsp; "
    f"🎯 CRD: <b style='color:#4A7090'>{crd_str}</b></p>"
    f"</div>",
    unsafe_allow_html=True,
)
