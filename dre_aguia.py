...
ESTRUTURA_DRE = [
    ...
    ("INVESTIMENTOS",                              "(memo) Investimentos e Financiamentos", -1),
]

SUBTOTAIS = {
    "(-) Comissão sobre Vendas":                 ("Venda Líquida",                      False),
    "(+) Reembolso de Custos":                   ("Margem de Contribuição",             False),
    "Impostos Incidentes s/ Receita Tributável": ("Receita Líquida (após impostos)",    True),
    "Despesas Administrativa, Comercial e Mkt":  ("Resultado Operacional (EBITDA)",     False),
    "(-) Despesas Financeiras":                  ("Resultado Líquido do Exercício",     True),
}

# Categorias que aparecem na DRE mas NÃO entram na soma do resultado.
# Renderizam como linha informativa (memo) após o Resultado Líquido.
# INVESTIMENTOS aqui é, na prática, ~78% financiamento + ~22% tributos/parcelamentos
# (e quase 0% CAPEX real) — movimentação de caixa de financiamento, não resultado econômico.
MEMO_LINHAS = {"INVESTIMENTOS"}
...
SECOES = {
    ...
    "INVESTIMENTOS":                             "INFORMATIVO — NÃO INTEGRA O RESULTADO",
}
...
xlsx_path    = os.path.join(diretorio, "base_orc_abril.xlsx")
...
def _build_waterfall_payload(df_full: pd.DataFrame, estrutura, subtotais,
                              target_subtotal: str, ano_sel: int) -> dict:
    """Calcula dados do waterfall para todos os pares de períodos possíveis.

    Retorna um dict com:
      - 'periods': lista de períodos disponíveis [{key, label, ano, mes}, ...]
      - 'values_by_period': {period_key: {dre_name: valor}}
        — pré-computa o valor de cada categoria em cada período
      - 'categories': lista [{nome, label, sinal}, ...] das categorias incluídas
        no caminho até `target_subtotal` (ordenadas conforme estrutura)
      - 'target': nome do subtotal alvo

    A decomposição (delta entre dois períodos) é feita no JavaScript a partir
    desses dados — assim o usuário pode trocar de períodos sem reload.
    """
    if df_full is None or df_full.empty:
        return {'periods': [], 'values_by_period': {}, 'categories': [], 'target': target_subtotal}

    # 1) Determina categorias do caminho até o target_subtotal
    categorias = []
    for nome_dre, label, sinal in estrutura:
        categorias.append({'nome': nome_dre, 'label': label, 'sinal': int(sinal)})
        # Verifica se este nome fecha o subtotal alvo
        if nome_dre in subtotais and subtotais[nome_dre][0] == target_subtotal:
            break

    # 2) Períodos: 12 meses do ano atual + 12 do ano anterior (se houver dados)
    anos_disponiveis = sorted(df_full['Ano'].unique().tolist())
    # Mantém só o ano corrente e o anterior para não inflar
    anos_relev = [a for a in anos_disponiveis if a in (ano_sel - 1, ano_sel)]

    periods = []
    values_by_period = {}

    for ano in anos_relev:
        for mes in range(1, 13):
            sub = df_full[(df_full['Ano'] == ano) & (df_full['Mes'] == mes)]
            if sub.empty:
                continue
            period_key = f"{ano}-{mes:02d}"
            label = f"{_MESES_NOMES[mes]}/{str(ano)[2:]}"
            periods.append({'key': period_key, 'label': label, 'ano': ano, 'mes': mes})

            # Calcula valor de cada categoria neste período
            vals = {}
            for cat in categorias:
                v = _brief_calc_cat(sub, cat['nome'], cat['sinal'])
                vals[cat['nome']] = float(v)
            values_by_period[period_key] = vals

    return {
        'periods': periods,
        'values_by_period': values_by_period,
        'categories': categorias,
        'target': target_subtotal,
    }
...
def _build_executive_brief(df_f, df_orc, df_orc_ano, df_yoy,
    ...
    # ─── 1. Métricas-chave ───────────────────────────────────────
    rec_real = _brief_calc_subtotal(df_f,    estrutura, 'Receita Líquida (após impostos)', subtotais)
    rec_orc  = _brief_calc_subtotal(df_orc,  estrutura, 'Receita Líquida (após impostos)', subtotais)
    rec_yoy  = _brief_calc_subtotal(df_yoy,  estrutura, 'Receita Líquida (após impostos)', subtotais)
    ...
    narrativa = (
        ...
        f"EBITDA fechou em <span class='ebitda-clickable' onclick='openWaterfall()' "
        f"style='color:var(--c-text-1,#E2E8F0);font-weight:600;cursor:pointer;"
        f"border-bottom:1px dashed var(--c-accent,#3B82F6);' "
        f"title='Clique para ver análise de variação'>{_brief_fmt(ebit_real)}</span> "
...
with st.sidebar:
    ...
    mostrar_informativas = st.toggle("ℹ️ Mostrar linhas informativas (Investimentos e Mov. de Caixa)", value=True)
...
def build_dre_html(df_f: pd.DataFrame, df_orc: pd.DataFrame, df_orc_ano: pd.DataFrame,
                   ...
                   ano_sel: int = None, mostrar_informativas: bool = True) -> str:
    ...
    def _td_orc(label: str, valor: float, orc: float, orc_ano: float = 0.0,
        ...
        # SETA (Opção B): direção numérica = a magnitude exibida subiu ou caiu.
        #   ▲ = magnitude aumentou   ▼ = magnitude diminuiu
        # COR (acima): mérito = favorável (verde) ou desfavorável (vermelho).
        # As duas dimensões são independentes: ex. despesa que caiu = ▼ verde.
        if valor * orc < 0:
            # Sinais opostos (cruzou o zero): magnitude fica ambígua,
            # então a seta segue o mérito — sobe se melhorou, desce se piorou.
            sinal_v = "▲" if not ruim else "▼"
        elif abs(valor) > abs(orc):
            sinal_v = "▲"
        elif abs(valor) < abs(orc):
            sinal_v = "▼"
        else:
    ...
    def processar_categoria(nome_dre, label_exib, sinal_pad):
        ...
        # Linhas MEMO (ex.: INVESTIMENTOS) aparecem na DRE mas NÃO entram na
        # soma do resultado — são movimentação de caixa de financiamento/tributos,
        # não resultado econômico. São renderizadas separadamente após o loop.
        eh_memo = nome_dre in MEMO_LINHAS
        if not eh_memo:
            acumulador += valor
        ...
        if valor == 0.0:
            if nome_dre in subtotais:
                ...
                is_final = label_sub == "Resultado Líquido do Exercício"
        ...
        if nome_dre in subtotais:
            ...
            for nome2, _, sp2 in estrutura:
                ...
                if nome2 == nome_dre:
                    break
            _eh_mc = (label_sub == "Margem de Contribuição")
    ...
    for nome_dre, label_exib, sinal_pad in estrutura:
        # Linhas informativas (memo) só aparecem se o toggle estiver ativo.
        # Marca como mapeada mesmo quando oculta, para não cair na seção de órfãs.
        if nome_dre in MEMO_LINHAS and not mostrar_informativas:
    ...
    # Serializa payload do waterfall (análise de variação do EBITDA)
    # Concatena df_f + df_prov + df_yoy para ter todos os meses dos 2 anos
    _frames = [f for f in (df_f, df_prov, df_yoy) if f is not None and not f.empty]
    _df_water_all = pd.concat(_frames, ignore_index=True) if _frames else pd.DataFrame()
    _waterfall_payload = _build_waterfall_payload(
        _df_water_all, estrutura, subtotais,
        target_subtotal='Resultado Operacional (EBITDA)',
        ano_sel=ano_sel or 2026
    )
    waterfall_json = _json.dumps(_sanitize(_waterfall_payload), ensure_ascii=True, default=str)
    waterfall_json_safe = waterfall_json.replace("</", "<\\/")
...
/* ── MODAL WATERFALL (análise de variação) ── */
#waterfall-modal-overlay {
  display: none; position: absolute; top: 0; left: 0; width: 100%;
  background: rgba(4,8,18,.75); backdrop-filter: blur(4px);
  z-index: 9999;
}
#waterfall-modal-overlay.open { display: block; }
#waterfall-modal-box {
  background: var(--c-bg-2,#0A1628); border: 1px solid var(--c-border-2,#1A3050); border-radius: 12px;
  box-shadow: 0 24px 64px rgba(0,0,0,.6);
...
#waterfall-modal-header {
  background: var(--c-bg-3,#060D1A); border-bottom: 1px solid var(--c-border-2,#1A3050);
  padding: 14px 20px; display: flex; align-items: center;
  justify-content: space-between; flex-shrink: 0;
}
#waterfall-modal-title { color: var(--c-text-1b,#CBD5E1); font-size: 14px; font-weight: 700; }
#waterfall-modal-sub   { color: var(--c-text-3b,#3D6080); font-size: 11.5px; margin-top: 2px; }
#waterfall-modal-close { background: none; border: none; color: var(--c-text-4,#2A5080); font-size: 22px; cursor: pointer; line-height: 1; padding: 0 4px; }
#waterfall-modal-close:hover { color: var(--c-accent-bright,#7DD3FC); }

#waterfall-controls {
  display: flex; align-items: center; gap: 14px;
  padding: 12px 20px 10px; background: var(--c-bg-3,#060D1A);
  border-bottom: 1px solid var(--c-border-1,#1A2E4A);
  font-family: 'Inter', sans-serif;
}
#waterfall-controls .wf-control { display: flex; align-items: center; gap: 8px; }
#waterfall-controls label { font-size: 10.5px; color: var(--c-text-3,#607A90); text-transform: uppercase; letter-spacing: .05em; font-weight: 600; }
#waterfall-controls select {
  background: var(--c-bg-2,#0A1628); border: 1px solid var(--c-border-2,#1A3050);
  color: var(--c-text-1b,#CBD5E1); font-size: 12px; font-weight: 500;
  padding: 5px 9px; border-radius: 5px; cursor: pointer; outline: none;
  font-family: 'Inter', sans-serif; transition: border-color .15s;
}
#waterfall-controls select:hover { border-color: var(--c-accent,#3B82F6); }
#waterfall-controls .wf-arrow { color: var(--c-text-4,#2A5080); font-size: 14px; }
#wf-summary {
  margin-left: auto; font-size: 12px; color: var(--c-text-1b,#CBD5E1);
  display: flex; align-items: center; gap: 8px;
}

#waterfall-modal-body { padding: 16px 20px 6px; background: var(--c-bg-1,#080F1C); }
#waterfall-svg { width: 100%; height: 360px; display: block; }

#waterfall-modal-legend {
  display: flex; gap: 18px; padding: 0 20px 10px;
  font-size: 11px; color: var(--c-text-3,#607A90); background: var(--c-bg-1,#080F1C);
}
#waterfall-modal-legend .wf-lg { display: inline-flex; align-items: center; gap: 6px; }
#waterfall-modal-legend .wf-sw { width: 12px; height: 12px; border-radius: 3px; display: inline-block; }

#waterfall-modal-footer {
  background: var(--c-bg-3,#060D1A); border-top: 1px solid var(--c-border-1,#1A2E4A);
  padding: 8px 20px; font-size: 10.5px; color: var(--c-text-4b,#2A4560);
}

.ebitda-clickable:hover { color: var(--c-accent-bright,#7DD3FC) !important; }

.wf-svg .wf-bar-total { fill: var(--c-text-3e,#475569); }
.wf-svg .wf-bar-pos { fill: var(--c-success,#22C55E); }
.wf-svg .wf-bar-neg { fill: var(--c-danger,#EF4444); }
.wf-svg .wf-conn { stroke: var(--c-text-4d,#3D5A78); stroke-width: 0.7; stroke-dasharray: 3,2; }
.wf-svg .wf-grid { stroke: var(--c-border-1,#1A2E4A); stroke-width: 0.5; stroke-dasharray: 2,3; }
.wf-svg .wf-axis { fill: var(--c-text-3e,#475569); font-family: 'Inter', sans-serif; font-size: 9.5px; }
.wf-svg .wf-cat-label { fill: var(--c-text-2,#94A3B8); font-family: 'Inter', sans-serif; font-size: 9.5px; }
.wf-svg .wf-val-label { font-family: 'Inter', sans-serif; font-size: 10px; font-weight: 600; }
.wf-svg .wf-total-label { fill: var(--c-text-1,#E2E8F0); font-family: 'Inter', sans-serif; font-size: 11px; font-weight: 600; }
</style>
...
var WATERFALL_DATA = """ + waterfall_json_safe + """;
...
function closeWaterfall(){ document.getElementById('waterfall-modal-overlay').classList.remove('open'); }
document.addEventListener('keydown',function(e){
  if(e.key==='Escape'){ closeModal(); closeChartModal(); closeWaterfall(); }
});
...
/* ── MODAL WATERFALL (análise de variação do EBITDA) ─────────────────── */

function _fmtWfFull(v) {
  var sign = v < 0 ? '-' : (v > 0 ? '+' : '');
  var s = Math.abs(v).toLocaleString('pt-BR', {minimumFractionDigits: 0, maximumFractionDigits: 0});
  return sign + 'R$ ' + s;
}

function _fmtWfCompact(v) {
  var sign = v < 0 ? '-' : (v > 0 ? '+' : '');
  var a = Math.abs(v);
...
function _wfChooseDefaultPeriods() {
  // Padrão inteligente: último mês com dados vs mês imediatamente anterior
  var periods = WATERFALL_DATA.periods || [];
  if (periods.length < 2) return [null, null];
  var n = periods.length;
  // B = último, A = penúltimo
  return [periods[n-2].key, periods[n-1].key];
}

function openWaterfall() {
  if (!WATERFALL_DATA || !WATERFALL_DATA.periods || WATERFALL_DATA.periods.length < 2) {
    alert('Não há dados suficientes para análise de variação (mínimo 2 períodos).');
    return;
  }

  // Popula os selectboxes
  var selA = document.getElementById('wf-period-a');
  var selB = document.getElementById('wf-period-b');
  selA.innerHTML = ''; selB.innerHTML = '';
  WATERFALL_DATA.periods.forEach(function(p) {
    var oA = document.createElement('option'); oA.value = p.key; oA.textContent = p.label;
    var oB = document.createElement('option'); oB.value = p.key; oB.textContent = p.label;
    selA.appendChild(oA); selB.appendChild(oB);
  });

  // Define períodos default
  var defaults = _wfChooseDefaultPeriods();
  selA.value = defaults[0];
  selB.value = defaults[1];

  // Listener para mudanças
  selA.onchange = renderWaterfall;
  selB.onchange = renderWaterfall;

  renderWaterfall();

  // Mostra modal
  var overlay = document.getElementById('waterfall-modal-overlay');
  overlay.style.height = Math.max(document.body.scrollHeight, document.documentElement.scrollHeight) + 'px';
  var box = document.getElementById('waterfall-modal-box');
  var scrollY = window.scrollY || window.pageYOffset || 0;
  box.style.top = (scrollY + 40) + 'px';
  overlay.classList.add('open');
}

function renderWaterfall() {
  var keyA = document.getElementById('wf-period-a').value;
  var keyB = document.getElementById('wf-period-b').value;
  var valsA = WATERFALL_DATA.values_by_period[keyA] || {};
  var valsB = WATERFALL_DATA.values_by_period[keyB] || {};
  var cats = WATERFALL_DATA.categories || [];
  var pA = WATERFALL_DATA.periods.find(function(p){ return p.key === keyA; });
  var pB = WATERFALL_DATA.periods.find(function(p){ return p.key === keyB; });

  // Fallback se algum período não existir no payload (ex: usuário trocou
  // filtro de ano e o período antigo não está mais disponível)
  if (!pA || !pB) {
    document.getElementById('waterfall-svg').innerHTML =
      '<text x="440" y="180" text-anchor="middle" fill="var(--c-text-3,#607A90)" ' +
      'font-family="Inter" font-size="12">Selecione dois períodos válidos para análise.</text>';
    document.getElementById('wf-summary').innerHTML = '';
    return;
  }

  // Calcula deltas + totais
  var totalA = 0, totalB = 0;
  var contribs = [];
  cats.forEach(function(c) {
    var vA = valsA[c.nome] || 0;
    var vB = valsB[c.nome] || 0;
    totalA += vA;
    totalB += vB;
    var delta = vB - vA;
    contribs.push({label: c.label, delta: delta, vA: vA, vB: vB});
  });

  var totalDelta = totalB - totalA;

  // Atualiza summary no header
  var deltaPct = totalA !== 0 ? (totalDelta / Math.abs(totalA) * 100) : 0;
  var arrow = totalDelta >= 0 ? '↑' : '↓';
  var deltaCor = totalDelta >= 0 ? 'var(--c-success,#22C55E)' : 'var(--c-danger,#EF4444)';
  var summaryEl = document.getElementById('wf-summary');
  summaryEl.innerHTML = '<span style="color:var(--c-text-3,#607A90);">Variação:</span> ' +
    '<span style="color:' + deltaCor + ';font-weight:600;font-size:13px;">' +
    _fmtWfFull(totalDelta) + '</span> ' +
    '<span style="color:' + deltaCor + ';">(' + arrow + ' ' + Math.abs(deltaPct).toFixed(1) + '%)</span>';

  // Renderiza SVG
  var W = 880, H = 360;
  var padL = 55, padR = 25, padT = 30, padB = 60;
  var plotW = W - padL - padR;
  var plotH = H - padT - padB;

  // Determina range Y considerando trajetória cumulativa (com os "running totals")
  var running = totalA;
  var minY = Math.min(0, totalA, totalB);
  var maxY = Math.max(0, totalA, totalB);
  ...
  function yPos(v) { return padT + plotH * (1 - (v - minY) / range); }
  var y0 = yPos(0);

  // Slots: 1 (total A) + N (contribs) + 1 (total B)
  var nSlots = 2 + contribs.length;
  var slotW = plotW / nSlots;
  var barW = slotW * 0.55;
  var barGap = (slotW - barW) / 2;

  var svg = '';

  // Grid + Y ticks (4 níveis)
  var gridVals = [];
  for (var i = 0; i <= 4; i++) {
    gridVals.push(minY + (range * i / 4));
  }
  gridVals.forEach(function(gv) {
    var yp = yPos(gv);
    svg += '<line class="wf-grid" x1="' + padL + '" y1="' + yp + '" x2="' + (W-padR) + '" y2="' + yp + '"/>';
    svg += '<text class="wf-axis" x="' + (padL - 6) + '" y="' + (yp + 3) + '" text-anchor="end">' + _fmtWfCompact(gv) + '</text>';
  });
  // Linha zero
  svg += '<line x1="' + padL + '" y1="' + y0 + '" x2="' + (W-padR) + '" y2="' + y0 + '" stroke="var(--c-text-4,#2A5080)" stroke-width="1"/>';

  // ─ Barra Total A (cinza, ancorada no chão)
  var slotIdx = 0;
  var cxA = padL + slotW * slotIdx + slotW / 2;
  var xA = cxA - barW / 2;
  var yTopA = yPos(totalA);
  var hA = Math.abs(yTopA - y0);
  svg += '<rect class="wf-bar-total" x="' + xA + '" y="' + Math.min(yTopA, y0) + '" width="' + barW + '" height="' + hA + '" rx="2">';
  svg += '<title>' + pA.label + ': ' + _fmtWfFull(totalA) + '</title></rect>';
  svg += '<text class="wf-total-label" x="' + cxA + '" y="' + (Math.min(yTopA, y0) - 5) + '" text-anchor="middle">' + _fmtWfCompact(totalA) + '</text>';
  svg += '<text class="wf-cat-label" x="' + cxA + '" y="' + (H - padB + 18) + '" text-anchor="middle" font-weight="600" fill="var(--c-text-1b,#CBD5E1)">' + pA.label + '</text>';

  // ─ Barras flutuantes (contribuições)
  var runVal = totalA;
  contribs.forEach(function(c, idx) {
    slotIdx = idx + 1;
    var cx = padL + slotW * slotIdx + slotW / 2;
    var x  = cx - barW / 2;
    var topVal = runVal + c.delta;
    var yTop = yPos(Math.max(runVal, topVal));
    var yBot = yPos(Math.min(runVal, topVal));
    var barH = Math.abs(yBot - yTop);
    var cls = c.delta >= 0 ? 'wf-bar-pos' : 'wf-bar-neg';
    var labelCor = c.delta >= 0 ? 'var(--c-success,#22C55E)' : 'var(--c-danger,#EF4444)';
    svg += '<rect class="' + cls + '" x="' + x + '" y="' + yTop + '" width="' + barW + '" height="' + Math.max(barH, 1) + '" rx="2">';
    svg += '<title>' + c.label + ': ' + _fmtWfFull(c.delta) + ' (de ' + _fmtWfFull(c.vA) + ' para ' + _fmtWfFull(c.vB) + ')</title></rect>';

    // Label do valor (acima da barra)
    svg += '<text class="wf-val-label" x="' + cx + '" y="' + (yTop - 5) + '" text-anchor="middle" fill="' + labelCor + '">' + _fmtWfCompact(c.delta) + '</text>';

    // Label da categoria (rotated 35° para caber)
    var labelTxt = c.label.length > 18 ? c.label.substring(0, 16) + '…' : c.label;
    svg += '<text class="wf-cat-label" x="' + cx + '" y="' + (H - padB + 14) + '" text-anchor="end" transform="rotate(-35 ' + cx + ' ' + (H - padB + 14) + ')">' + labelTxt + '</text>';

    // Linha conectora entre o topo desta barra e a base da próxima
    var nextX = padL + slotW * (slotIdx + 1) + slotW / 2 - barW / 2;
    var connY = yPos(topVal);
    svg += '<line class="wf-conn" x1="' + (x + barW) + '" y1="' + connY + '" x2="' + nextX + '" y2="' + connY + '"/>';

    runVal = topVal;
  });

  // ─ Barra Total B (cinza, ancorada no chão)
  slotIdx = nSlots - 1;
  var cxB = padL + slotW * slotIdx + slotW / 2;
  var xB = cxB - barW / 2;
  var yTopB = yPos(totalB);
  var hB = Math.abs(yTopB - y0);
  svg += '<rect class="wf-bar-total" x="' + xB + '" y="' + Math.min(yTopB, y0) + '" width="' + barW + '" height="' + hB + '" rx="2">';
  svg += '<title>' + pB.label + ': ' + _fmtWfFull(totalB) + '</title></rect>';
  svg += '<text class="wf-total-label" x="' + cxB + '" y="' + (Math.min(yTopB, y0) - 5) + '" text-anchor="middle">' + _fmtWfCompact(totalB) + '</text>';
  svg += '<text class="wf-cat-label" x="' + cxB + '" y="' + (H - padB + 18) + '" text-anchor="middle" font-weight="600" fill="var(--c-text-1b,#CBD5E1)">' + pB.label + '</text>';

  var svgEl = document.getElementById('waterfall-svg');
  svgEl.setAttribute('class', 'wf-svg');
  svgEl.innerHTML = svg;
}
...
<!-- MODAL DE WATERFALL (análise de variação MoM/YoY do EBITDA) -->
<div id="waterfall-modal-overlay" onclick="if(event.target===this)closeWaterfall()">
  <div id="waterfall-modal-box">
    <div id="waterfall-modal-header">
      <div>
        <div id="waterfall-modal-title">Análise de variação — EBITDA</div>
        <div id="waterfall-modal-sub">Decomposição da variação por categoria</div>
      </div>
      <button id="waterfall-modal-close" onclick="closeWaterfall()">&#x2715;</button>
    </div>
    <div id="waterfall-controls">
      <div class="wf-control">
        <label>Comparar</label>
        <select id="wf-period-a"></select>
      </div>
      <div class="wf-arrow">→</div>
      <div class="wf-control">
        <label>com</label>
        <select id="wf-period-b"></select>
      </div>
      <div id="wf-summary"></div>
    </div>
    <div id="waterfall-modal-body">
      <svg id="waterfall-svg" viewBox="0 0 880 360" preserveAspectRatio="xMidYMid meet"></svg>
    </div>
    <div id="waterfall-modal-legend">
      <span class="wf-lg"><span class="wf-sw" style="background:var(--c-text-3e,#475569);"></span> Totais</span>
      <span class="wf-lg"><span class="wf-sw" style="background:var(--c-success,#22C55E);"></span> Contribuição favorável</span>
      <span class="wf-lg"><span class="wf-sw" style="background:var(--c-danger,#EF4444);"></span> Contribuição desfavorável</span>
    </div>
    <div id="waterfall-modal-footer">
      <span>A soma das contribuições é exatamente igual à variação do EBITDA (decomposição matematicamente fechada).</span>
    </div>
  </div>
</div>
...
/* ── Barras de composição ── */
var desp_keys = [
  { label:'Impostos',      key:'Receita Líquida (após impostos)',   cor:'#D85A30' },
...
var rl   = R['Receita Líquida (após impostos)']||0;
...
(function(){
  ...
  var MC = R['Margem de Contribui\u00e7\u00e3o']||0;
...
(function(){
  ...
  var LABELS = {
    ...
    'Receita Líquida (após impostos)':      'Receita Líquida (após impostos)',
  ...
  var SEQ = [
    ...
    { tipo:'deriv', lbl:'(-) Custo das Vendas',      de:'Margem de Contribuição',              ate:'Venda Líquida' },
    { tipo:'sub',   lbl:'Margem de Contribuição',    key:'Margem de Contribuição' },
    { tipo:'deriv', lbl:'(-) Impostos s/ Receita',   de:'Receita Líquida (após impostos)',     ate:'Margem de Contribuição' },
    { tipo:'sub',   lbl:'Receita Líquida (após imp.)', key:'Receita Líquida (após impostos)' },
    { tipo:'deriv', lbl:'(-) Despesas Operacionais', de:'Resultado Operacional (EBITDA)',       ate:'Receita Líquida (após impostos)' },
...
html_dre, resumo_executivo, altura_dre = build_dre_html(
    ...
    mostrar_informativas=mostrar_informativas
)
